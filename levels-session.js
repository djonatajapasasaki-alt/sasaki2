(function (root, factory) {
  ...
  function exportTrydIndicator(session) {
    ...
    const levels = session.levels.map((level, index) => {
      ...
      const asset = String(level.asset || 'ATIVO').toUpperCase();
      const event = String(level.event || 'NIVEL').toUpperCase();
      const namedBase = `${asset} | ${event} | ${base}`;
      const count = (usedLabels.get(namedBase) || 0) + 1;
      usedLabels.set(namedBase, count);
      const uniqueLabel = count > 1 ? `${namedBase} | ${count}` : namedBase;
      return { price: Number(level.price), label: uniqueLabel, color: level.color || '#FFFFFF', width: Math.max(1, Math.min(10, Number(level.width) || 1)) };
    });
...
// Cada linha mantém o nome do indicador no Tryd.
